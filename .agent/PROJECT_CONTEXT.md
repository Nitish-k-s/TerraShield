...
├── package.json
├── tsconfig.json
├── middleware.ts                      # Auth guard: redirect unauthed → /login
│
├── app/
│   ├── layout.tsx                     # Root layout (barebones — needs update)
│   ├── page.tsx                       # Test page: upload image → EXIF + Gemini analysis
│   ├── login/
│   │   └── page.tsx                   # Login / sign-up form (Supabase email+password)
│   └── auth/
│       └── callback/
...
│       └── analyse-exif/
│           └── route.ts               # POST: { recordId } or { pending: true } → Gemini AI
│
├── lib/
│   ├── supabase/
...
│   └── db/
│       ├── exif.ts                    # SQLite schema, CRUD helpers (insertExifRecord, updateAiAnalysis, etc.)
...
| `app/layout.tsx` has placeholder metadata ("Generated by Next.js") | Should be updated with TerraShield branding |
